		...
		<div class="goods__list goods__list-2">
			...
			<div class="goods__item goods__item_list">
				...
				<div class="goods__img">
					<a href="<?=$item['DETAIL_PAGE_URL'];?>"><img src="<?=$item['PREVIEW_PICTURE']['SRC']?>" alt="<?=$item['NAME']?>" title="<?=$item['NAME']?>"></a>
				</div>
				<div class="goods__content">
					...
					<? if($item['PROPERTIES']['BADGE']['VALUE']): ?>
						<div class="goods__availability true" style="background-color: <?=$item['PROPERTIES']['BADGE']['DESCRIPTION']?>"><?=$item['PROPERTIES']['BADGE']['VALUE'];?></div>
					<? endif; ?>
				</div>
				<div class="goods__main">
					...
					<?else:
						$incPriceReq++;
						?>
						<? if($incPriceReq > $arResult["UF_HIDE_PRICE_REQ"] && !is_null($arResult["UF_HIDE_PRICE_REQ"])): ?>
                    ...
                    <? if(count($item['PROPERTIES']['ARTICLS']['VALUE']) > 1): ?>
						<? if($let >= $arResult["UF_HIDE_BUY"] && !is_null($arResult["UF_HIDE_BUY"])): ?>
                            <a href="javascript:void(0)" class="goods__buy" onclick="$('.more_options').bPopup({ zIndex:1000, loadUrl: '/ajax/popup_option.php?ID=<?=$item['ID']?>' }).css({width: '100%'});" data-text="Купить"></a>
                        <? else: ?>
                            <a href="javascript:void(0)" class="goods__buy" onclick="$('.more_options').bPopup({ zIndex:1000, loadUrl: '/ajax/popup_option.php?ID=<?=$item['ID']?>' }).css({width: '100%'});">Купить</a>
                        <? endif; ?>
                    <?else:?>
