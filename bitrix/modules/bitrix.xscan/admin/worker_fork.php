...
if ($action === 'showfile') {
    ...
    if (!$file || !file_exists($file)) {
        ...
        echo substr($file, -4) !== '.ph_' ? CBitrixXscanFork::getIsolateButton($file) : CBitrixXscanFork::getUnIsolateButton($file);
        echo CBitrixXscanFork::getHideButton($file);

        if (strpos($file, $_SERVER['DOCUMENT_ROOT']) === 0){
            $p = substr($file, strlen($_SERVER['DOCUMENT_ROOT']));
            echo '<a target="blank" class="ui-btn ui-btn-primary ui-btn-sm" href="fileman_file_edit.php?path='.urlencode($p).'&full_src=Y">'.GetMessage("BITRIX_XSCAN_EDIT_BTN").'</a>';
        }
...
<script>
    ...
    function income_event(a) {
        ...
        if (a.eventId == 'xscan-grid') {
            result = JSON.parse(a.data.result);
            BX('alert_msg').innerHTML = result['msg'] || result['error'];
            GridRenew();
        }
    }
...
$list = \CBitrixXscanFork::getList($filter, $inprogress, $nav, $sort);
$total = \CBitrixXscanFork::getTotal($filter);
