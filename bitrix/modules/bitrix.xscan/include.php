...
//			"url" => "partner_modules.php?module=".$MODULE_ID,
        ...
        if (file_exists($path = dirname(__FILE__) . '/admin')) {
            if ($dir = opendir($path)) {
                ...
                $texts = [
                    'help.php' => GetMessage("BITRIX_XSCAN_HELP"),
                    'worker.php' => GetMessage("BITRIX_XSCAN_SEARCH_OLD"),
                    'worker_fork.php' => GetMessage("BITRIX_XSCAN_SEARCH_FORK"),
                    'system.php' => GetMessage("BITRIX_XSCAN_SYSTEM")
                ];

                foreach ($arFiles as $item)
                    $aMenu['items'][] = array(
                        'text' => $texts[$item] ?? $item,
