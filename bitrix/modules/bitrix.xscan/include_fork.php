...
class CBitrixXscanFork
{
    ...
    public $false_positives = ['b4cd0dcca178dbc12935e505860864ad', 'f9577fd4ffb4024a8b1c7a63247e52e7', 'cc6969f6a56325d5676bc02a268057f8',
        ...
        'a5b078e8aedc3e9ffa127487350f1111', '0560f452bafbdbc357734cc69f54d1d2', '10b825cc41ea378aa928573a7f3dd60f', '16bc692d1658a8478d568b9075663a57',
        'b3062ee996247ba09b5a29dfa1b3afea', '9d26d90f3970187c3f748573978d868d', '730e7fea28e73713bcab71fe6678f906', 'cf7fb7c49317a186b741adec8ae4005a',
        '463ce09267c3c966dda3144aa3ef75d4', 'fc81ad62d76829d50e02ca8abc896480', 'bb51a75da5892416d8a38da7243380c1'];
            ...
            foreach ($regs[1] as $i => $val) {
                ...
                if (preg_match($spl . preg_quote($spiltter) . '[imsxADSUXju]*e[imsxADSUXju]*[\'"]' . $spl, $val)) {
                    ...
                    if (!$this->IsFalsePositive($checksum)) {
                        $this->addResult($status, $code, self::CaclCrit($status), $checksum);
                    }
                }
            }
        }
...
//                        );
        ...
        foreach ($nodes['variables'] as $var) {
            ...
            if (preg_match('#\$_{3,}#i', $v)) {
                ...
                if (!$this->IsFalsePositive($checksum)) {
                    $this->addResult($subj, $v, self::CaclCrit($subj), $checksum);
                }
...
//				echo $var." => ".$p."<br>";
    ...
    static function getList($filter, $inprogress, $nav, $sort)
    {
        $output = [];
