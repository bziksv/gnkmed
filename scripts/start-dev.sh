...
if mysqladmin --protocol=SOCKET ping --silent 2>/dev/null; then
  ...
  TABLES=$(mysql -h 127.0.0.1 -u gnkmed_local -pgnkmed_local -N -e \
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='gnkmed_ru_db'" 2>/dev/null || echo 0)
  if [ "$TABLES" -lt 50 ]; then
    ...
    echo "Local MySQL: gnkmed_ru_db ($TABLES tables)"
  fi
else
...
HTTP=$(curl -sS -o /tmp/gnkmed-check.html -w '%{http_code}' --max-time 60 http://127.0.0.1:8095/ || echo 000)

echo "soft http://127.0.0.1:8095/ → HTTP $HTTP"
echo "php-fpm :9095 (ondemand, max 4 workers, 512M)"
